...
static char COORD_IF_snsrDataJSONSerialFmt[Q3_MSG_Q_BUFF_LEN];
...
DynamicJsonDocument CLOUD_IF_wsnMsgJSONDoc(512);
...
uint16_t COORD_IF_mapExt2ShortAddr(const uint8_t *extAddr_p)
{
   return WSN_SHORT_BROADCAST_ADDR;
}
...
{  
  ...
  sprintf(NODE_macIdStr, "%02x:%02x:%02x:%02x", 
          nodeEA_p[4], nodeEA_p[5], nodeEA_p[6], nodeEA_p[7]);
  ...
  COORD_IF_snsrDataJsonObj["MacId"] = NODE_macIdStr;
  COORD_IF_snsrDataJsonObj["RSSI"] = rssi;
  COORD_IF_snsrDataJsonObj["LQI"] = lqi;

  int idx = 0;
  for (; idx<snsrDataCnt; idx++)
  {
      char snsrIdStr[6];
      char snsrOpStr[16];
  ...
  serializeJson(COORD_IF_snsrDataJSONDoc, 
                COORD_IF_snsrDataJSONSerialFmt);

  Serial.printf("\n\nGW_bFSD> C JSON M: ");
  Serial.printf(COORD_IF_snsrDataJSONSerialFmt);
  Serial.printf("\n\n");

  // Send a message with extended MAC address + serialized JSON message + null character

  int jsonMsgLen = strlen(COORD_IF_snsrDataJSONSerialFmt) + 1;  // We will keep the terminating NULL character
  char *msg_p = (char*)malloc(Q3_MSG_Q_BUFF_LEN);
  if (msg_p == NULL)
  {
  ...
  char *jsonMsgPtr_p = msg_p + DIS_LPWMN_MAC_EXT_ADDR_LEN;  
  memcpy(jsonMsgPtr_p, COORD_IF_snsrDataJSONSerialFmt, jsonMsgLen);
  Serial.printf("GW_bFSD> jML: %d\n", jsonMsgLen);
  Serial.printf("GW_bFSD> jMsg: ");
  Serial.printf(jsonMsgPtr_p);
  Serial.printf("\n");
  
  if (xQueueSend(Q3Hndl, 
                (void*)msg_p, 
                (TickType_t)5) == pdPASS)
  {
      COORD_IF_q3EnqOkCnt ++;
...
{
  static uint32_t COORD_IF_pktDropNoCrCnt = 0,
                  COORD_IF_pktDropBLCnt = 0;
...
#if 1
  Serial.printf("C_I_pNM> #%u Rx M %d/%x:%x:%x:%x:%x:%x:%x:%x\n",
                COORD_IF_nodeMsgCnt,
                srcShortAddr,
  ...
  if (GW_checkNodeBlackListed(extAddr_p) == true)
  {
      COORD_IF_pktDropBLCnt ++;
      Serial.printf("C_I_pNM> N B-L #%u !!\n", COORD_IF_pktDropBLCnt);
      return;
  }

  if (GW_getNodeCntxt(extAddr_p) == NULL)
  {
      COORD_IF_pktDropNoCrCnt ++;
      Serial.printf("C_I_pNM> D P #%u !!\n", COORD_IF_pktDropNoCrCnt);

      GW_addNodeToPendingCredsList(extAddr_p);
      
      // Request dashboard to send credentials for this node if node is
      // provisioned.  
  ...
  {
      Serial.printf("C_I_pNM> dis-msg-type <0x%x> \n", disMsgType);
      switch(disMsgType)
      {
          case DIS_MSG_TYPE_GET_ATTR_VAL:
              {
                  Serial.printf("C_I_pNM> Msg Type - DIS_MSG_TYPE_GET_ATTR_VAL !! \n");
                  if (msgPyldLen >= DIS_TLV_HDR_SZ)
                  {
                      ...
                      if (rc == 0)
                      {
                          Serial.printf("C_I_pNM> Could not find DIS_TLV_TYPE_ATTR_INFO  !!\n");
                      }
                      else
                      {
                          Serial.printf("C_I_pNM> Found DIS_TLV_TYPE_ATTR_INFO - pyldLen<%d>/tlvLen1<%d> \n", msgPyldLen, tlvLen1);
                          ...
                          if (msgPyldLen >= tlvLen1)
                          {
                              ...
                              if (rc == 0)
                              {
                                   Serial.printf("C_I_pNM> Could not find DIS_TLV_TYPE_ATTR_ID  !!\n");
                              }
                              else
                              {
                                   Serial.printf("C_I_pNM> Found DIS_TLV_TYPE_ATTR_ID ... \n");
    
                                  if (tlvLen2 == DIS_ATTR_ID_FIELD_SZ)
                                  {
                                      unsigned int attrId = UTIL_ntohs(buff2_p);
  
                                      Serial.printf("C_I_pNM> AttrId<%u>  \n", attrId);
                                      ...
                                      {
                                          Serial.printf("C_I_pNM> Found DIS_TLV_TYPE_ATTR_VAL ... %u \n", tlvLen2);
                                          ...
                                          switch (tlvLen2)
                                          {
                                              ...
                                              case 1:
                                                ...
                                                {
                                                    Serial.printf("C_I_pNM> AttrVal<%d> \n", (char)(*buff2_p));
                                                    sprintf(CoordAttrRcvdData, "%d", (char)(*buff2_p));
                                                    if (xQueueSend(Q6Hndl, (void*)&CoordAttrRcvdData, (TickType_t)0) == pdPASS)
                                                    {
                                                         Serial.printf("C_I_pNM> Coord Resp passed into Q6Hndl..\n");
                                                    }
                                                    else
                                                    {
                                                         Serial.printf("C_I_pNM> No queue space in Q6Hndl...\n");
                                                    }
                                                }
                                                else
                                                {
                                                    Serial.printf("C_I_pNM> AttrVal<%u / 0x%x> \n", *buff2_p, *buff2_p);
                                           ...
                                           case 4:
                                            ...
                                            default:
                                                ...
                                                else
                                                {
                                                    ...
                                                    {
                                                        Serial.printf("C_I_pNM> AttrVal<%d byte response> \n", tlvLen2);
                                   ...
                                   {
                                       Serial.printf("C_I_pNM> Bad DIS_TLV_TYPE_ATTR_ID TLV !!\n");
                                   }
                                }
                            }
                      }
      ...
      case UART_MSG_TYPE_ACK:
           {
               ...
               {
                   hdrAcked = 1;
                   Serial.printf("C_I_pRF> Header Acked hdr flag<0x%02x> !! \n", hdrFlags);
                   if (xQueueSend(Q5Hndl, (void*)&hdrAcked, (TickType_t)100) == pdPASS)
                   {
                       Serial.printf("C_I_pRF> HDR Acked resp passed to Q5Hndl..\n");
                   }
                   else
                   {
                       Serial.printf("C_I_pRF> No queue space in Q5Hndl...\n");
                   }
               }
               else
               {
                   Serial.printf("C_I_pRF> hdr flag <0x%02x> hdrAcked <0> !! \n", hdrFlags);
                   hdrAcked = 0;
               }
           }
           break;
                   ...
                   case 1:
                        {
                           if (GW_coordPendingAttrId == CC1120_AGC_CS_THR_ATTR_ID)
                               Serial.printf("C_I_pRF> GET_COORD_ATTR_VAL: %d \n",  
                                                                        *((char *)(COORD_INTF_taskRxMsgBuff + UART_FRAME_HDR_LEN)));
                           else
                               Serial.printf("C_I_pRF> GET_COORD_ATTR_VAL: %u / 0x%x \n",
                           ...
                           if (xQueueSend(Q6Hndl, (void*)&CoordAttrRcvdData, (TickType_t)0) == pdPASS)
                           {
                               Serial.printf("C_I_pRF> Coord Resp passed into Q6Hndl..\n");
                           }
                           else
                           {
                               Serial.printf("C_I_pRF> No queue space in Q6Hndl...\n");
                           }
                        }
                        break;
                   ...
                   case 4:
                        {
                           ...
                           {
                               default:
                                      Serial.printf("C_I_pRF> Coord_Attr_Val: <%u / 0x%x> \n",
                                 ...
                                 uint8_t *seqNrUsed_p)
{
   ...
   static uint8_t COORD_IF_frameHdrSeqNr = 0x20;
   int rc = 0;
   ...
   *buff_p = COORD_IF_frameHdrSeqNr;
   if (seqNrUsed_p != NULL)
       *seqNrUsed_p = COORD_IF_frameHdrSeqNr;
   COORD_IF_frameHdrSeqNr ++;
   buff_p += UART_FRAME_HDR_SEQ_NR_FIELD_LEN;

   UTIL_htons(buff_p, pyldLen);
   buff_p += UART_FRAME_HDR_PYLD_LEN_FIELD_LEN;

   uint16_t calcCrc16 = COORD_IF_calcCkSum16(COORD_IF_serTxBuff, 
                                             UART_FRAME_HDR_HDR_CRC_FIELD_OFF);
...
SemaphoreHandle_t CLOUD_IF_nodeListPendingCredsMutexHndl;
...
void setup()
{
  ...
  CLOUD_IF_nodeListPendingCredsMutexHndl = xSemaphoreCreateMutex();
  configASSERT(CLOUD_IF_nodeListPendingCredsMutexHndl);
...
#ifdef WSN_COORD_SIM_ENA
#if 0
  GW_allocNodeCntxt(WSN_COORD_SIM_nodeAExtAddr,
                    WSN_COORD_SIM_nodeAMqttConnUser,
                    NULL);

  GW_allocNodeCntxt(WSN_COORD_SIM_nodeBExtAddr,
...
{ 
   ...
   static uint32_t loopEntryCnt = 0, loopExitCnt = 0;
   static uint32_t __wifiBeginCnt = 0;

   loopEntryCnt ++;
   //Serial.printf("L> Entry #%u\n", loopEntryCnt);
   ...
   //Serial.printf("L> WiFi C\n");

   CLOUD_IF_procNodesPendingCredsList();
